    ...
    const legendItems = [
      ...
      export const Initialize = () => {
        ...
        const handleImageClick = (x: number, y: number) => {
            if (selectedPiece) {
              initializeGame(selectedPiece, x, y, currentPlayer);
            }
          };
        ...
        async function initializeGame(pieceName: string, xFromSquare: number, yFromSquare: number, playerId: number) {
            ...
            if (response.ok) {
                ...
                // Check if the current player has placed all their pieces
                if (updatedGameState.players[currentPlayer - 1].piecesPlaced === 40) {
                    // If so, switch to the next player
                    switchPlayer();
                }
            } else {
        ...
        {renderLegend()}
                ...
                <div style={{ flex: 2 }}>
                    ...
                    <div style={{ maxWidth: '100%', padding: '0 16px', margin: '0 auto' }}>
                        ...
                        <GameGrid gameState={gameState} imageMapping={imageMapping} onImageClick={handleImageClick} />
                        </div>
